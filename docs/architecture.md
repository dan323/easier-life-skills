...
│   │   ├── source-tag.ts    Marketplace source tag component
│   │   ├── card-plugin.ts   Plugin card (clickable → opens detail panel)
│   │   ├── card-skill.ts    Skill card
│   │   ├── card-agent.ts    Agent card
│   │   ├── card-mcp.ts      MCP server card
│   │   ├── card-command.ts  Command card
│   │   ├── card-hook.ts     Hook card (shows event trigger chips)
│   │   ├── card-bundle.ts   Bundle card
│   │   ├── utils.ts         copyText, titleCase, makeExpandable helpers
...
├── tsconfig.web.json        TypeScript config for assets/src/ (bundler resolution, DOM lib)
...
assets/src/app.ts      →  esbuild         →  assets/bundle.js                  (gitignored)
```
