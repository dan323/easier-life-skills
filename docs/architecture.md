...
**Search form.** `Controls.tsx`'s `<form role="search">` filters live on every `onInput`, but the form also carries an explicit `<button type="submit" class="sr-only">Search</button>` and an `onSubmit={e => e.preventDefault()}` handler so keyboard / AT users can fire the form via Enter (WCAG 3.2.2) without the page rewriting the URL with a stray `?`. The input's `title` and `aria-label` both explicitly state that plain-English queries are accepted alongside exact name searches.

**Natural-language search.** When the query contains a space (`isNLQuery` in `assets/src/search.ts`) the app fires `nlSearch`, which tokenises the query and scores each entity against a pre-built inverted index loaded from `assets/search-index.json`. Entities whose token overlap passes the score threshold are ranked by relevance; the rest are hidden. Single-word queries fall back to a fast substring `textMatch` against each entity's name, description, and keywords — no index fetch needed. This means users can type either an exact name (e.g. `changelog`) or a plain-English description (e.g. `find unused code`) and get relevant results either way.
