...
│   │   ├── app.ts           Boot + event wiring (esbuild entry point)
│   │   ├── api.ts           Fetches skills_index.json from GitHub
│   │   ├── state.ts         Typed shared state (plugins, skills, agents, mcpServers, bundles)
│   │   ├── marketplace.ts   Loads marketplace index into state
│   │   ├── panel.ts         Plugin detail slide-in panel
│   │   ├── render.ts        Renders plugin/skill/agent/mcpServer/bundle grids + filters
│   │   ├── components.ts    DOM builders for cards and source tags
...
8. The website loads `skills_index.json` at runtime. The marketplace list is fixed at build time.
